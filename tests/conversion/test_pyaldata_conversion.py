...
def test_nwb_to_pyaldata(session_name):
    ...
    expected_columns = ['animal', 'session', 'trial_id', 'trial_name',
                        'trial_length', 'bin_size', 'idx_trial_start', 'idx_trial_end', 'values_before_camera_trigger', 'idx_before_camera_trigger',
       'idx_motion', 'idx_quiet_period_end', 'values_Sol_direction',
       'idx_Sol_direction', 'idx_quiet_period_start', 'idx_session_timer',
       'motion_sensor_xy', 'SSp_ul_chan_best', 'SSp_ul_KSLabel',
       'SSp_ul_spikes', 'SSp_m_chan_best', 'SSp_m_KSLabel', 'SSp_m_spikes',
       'MOp_chan_best', 'MOp_KSLabel', 'MOp_spikes', 'ccg_chan_best',
       'ccg_KSLabel', 'ccg_spikes', 'ccb_chan_best', 'ccb_KSLabel',
       'ccb_spikes', 'scwm_chan_best', 'scwm_KSLabel', 'scwm_spikes',
       'CP_chan_best', 'CP_KSLabel', 'CP_spikes']

    # Assert that all expected columns are present
    # TODO: Add some more thorough checking on the data structure.

    assert set(expected_columns).issubset(df.columns), f"Missing columns: {set(expected_columns) - set(df.columns)}"
